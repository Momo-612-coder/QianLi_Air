...
namespace ego_planner
{
                                        ...
                                        double local_speed_limit)
  {
    ...
    const double plan_vel = std::max(0.3, std::min(pp_.max_vel_, local_speed_limit > 1e-3 ? local_speed_limit : pp_.max_vel_));

    /*** STEP 1: INIT ***/
    double ts = (start_pt - local_target_pt).norm() > 0.1 ? pp_.ctrl_pt_dist / plan_vel * 1.5 : pp_.ctrl_pt_dist / plan_vel * 5; // pp_.ctrl_pt_dist / pp_.max_vel_ is too tense, and will surely exceed the acc/vel limits
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        double time = pow(plan_vel, 2) / pp_.max_acc_ > dist ? sqrt(dist / pp_.max_acc_) : (dist - pow(plan_vel, 2) / pp_.max_acc_) / plan_vel + 2 * plan_vel / pp_.max_acc_;
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        double poly_time = (local_data_.position_traj_.evaluateDeBoorT(t) - local_target_pt).norm() / plan_vel * 2;
        if (poly_time > ts)
        {
  ...
  bool EGOPlannerManager::planGlobalTrajWaypointsWithVels(const Eigen::Vector3d &start_pos, const Eigen::Vector3d &start_vel, const Eigen::Vector3d &start_acc,
                                                          const std::vector<Eigen::Vector3d> &waypoints, const std::vector<Eigen::Vector3d> &waypoint_vels,
                                                          const Eigen::Vector3d &end_acc)
  {
    if (waypoints.empty() || waypoints.size() != waypoint_vels.size())
      return false;

    vector<Eigen::Vector3d> pos, vel, acc;
    pos.reserve(waypoints.size() + 1);
    vel.reserve(waypoints.size() + 1);
    acc.reserve(waypoints.size() + 1);

    pos.push_back(start_pos);
    vel.push_back(start_vel);
    acc.push_back(start_acc);

    for (size_t i = 0; i < waypoints.size(); ++i)
    {
      pos.push_back(waypoints[i]);
      vel.push_back(waypoint_vels[i]);
      acc.push_back(i == waypoints.size() - 1 ? end_acc : Eigen::Vector3d::Zero());
    }

    Eigen::VectorXd time(pos.size() - 1);
    for (size_t i = 0; i < pos.size() - 1; ++i)
    {
      const double dist = (pos[i + 1] - pos[i]).norm();
      const double mean_vel = max(0.5 * (vel[i].norm() + vel[i + 1].norm()), 1e-3);
      const double cruise_time = dist / mean_vel;
      const double vel_change_time = (vel[i + 1] - vel[i]).norm() / max(pp_.max_acc_, 1e-3);
      time(static_cast<int>(i)) = max(0.5, max(cruise_time, vel_change_time) * 1.2);
    }

    PolynomialTraj gl_traj = PolynomialTraj::multi_segment_traj_gen(pos, vel, acc, time);
    auto time_now = ros::Time::now();
    global_data_.setGlobalTraj(gl_traj, time_now);
