...
PolynomialTraj PolynomialTraj::multi_segment_traj_gen(const std::vector<Eigen::Vector3d> &pos,
                                                      ...
                                                      const Eigen::VectorXd &time)
{
  ...
  if (pos.size() < 2 || pos.size() != vel.size() || pos.size() != acc.size() || time.size() != static_cast<int>(pos.size()) - 1)
  {
    std::cerr << "[PolynomialTraj]: invalid multi-segment input." << std::endl;
    return poly_traj;
  }

  for (size_t i = 0; i < pos.size() - 1; ++i)
  {
    PolynomialTraj seg = one_segment_traj_gen(pos[i], vel[i], acc[i], pos[i + 1], vel[i + 1], acc[i + 1], time(static_cast<int>(i)));

    poly_traj.addSegment(seg.cxs.front(), seg.cys.front(), seg.czs.front(), time(static_cast<int>(i)));
  }
