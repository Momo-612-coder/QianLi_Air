#ifndef _PWM_SE3_CONTROLLER_HPP_
#define _PWM_SE3_CONTROLLER_HPP_
...
bool last_manual_control_state = false;
...
struct DroneParam
{
    ...
    double max_throttle; // 最大油门，单位为0-1之间
    double max_torque; // 最大力矩，单位为Nm
};

static Eigen::Vector3d quatToEuler(const Eigen::Quaterniond& q); // 返回z-y-x顺序的欧拉角，单位为弧度

class PwmSe3Controller
{
private:
    ...
    ros::Subscriber start_state_suber_; // 起飞准备就绪订阅

    bool is_started_ = false; // 是否已经开始控制
    bool is_control_thread_running_ = false; // 控制线程是否正在运行
    ros::Time start_time_; // 控制开始时间
    ...
    void start_state_cb(const std_msgs::Header::ConstPtr& msg); // 起飞准备就绪回调函数

    void SE3Controller_init();
    ...
    Eigen::Vector3d smooth_target_velocity_;
    Eigen::Vector3d smooth_target_acceleration_;
    bool has_smooth_target_ = false;
    Eigen::Vector3d target_eulerAngle_; // roll, pitch, yaw
    double target_yaw_; // 目标航向角，单位为弧度
...
public:
    PwmSe3Controller(ros::NodeHandle *nh);
    ~PwmSe3Controller();
